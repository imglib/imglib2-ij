...
public class FlatKDTreeFlat
{


	public static class KDTree<T> {

        final KDTreeImplFlat impl;

        final Supplier<IntFunction<T>> valuesSupplier;

        KDTree(final KDTreeImplFlat impl, final Supplier<IntFunction<T>> valuesSupplier) {
            this.impl = impl;
    ...
    public static <T> KDTree<T> kdtree(
        ...
        double[] positions = new double[numDimensions * numPoints];
        {
            ...
            for (RealLocalizable point : points) {
                for (int d = 0; d < numDimensions; d++)
                    positions[numDimensions * i + d] = point.getDoublePosition(d);
                ++i;
            }
        }

        final int[] tree = KDTreeBuilderFlat.tree(positions, numDimensions);
        positions = KDTreeBuilderFlat.reorder(positions, tree);
        final List<T> reorderedValues = new ArrayList<>();
        KDTreeBuilderFlat.reorder(values::get, tree).forEachRemaining(reorderedValues::add);
        ...
        if (type instanceof NativeType) {
            ...
            return new KDTree<>(new KDTreeImplFlat(positions, numDimensions), valuesSupplier);
        }
    ...
    public static void main(String[] args) {
        ...
        NearestNeighborSearch<ARGBType> search = new NearestNeighborSearchOnKDTreeFlat<>(kdtree);
